...
channel_names=['Fp1','AF3','F3','F7','FC5','FC1','C3','T7','CP5','CP1','P3','P7','PO3','O1','Oz',
...
class predictEmotion(object): 
	...
	def predict_emotion(self,feature):
		...
		idx_nearest_ar = np.array(np.argsort(distance_ar)[:2])
		val_nearest_ar = np.array(np.sort(distance_ar)[:2])

		#Compute 3 nearest index and distance value from arousal
		idx_nearest_va = np.array(np.argsort(distance_va)[:2])
		val_nearest_va = np.array(np.sort(distance_va)[:2])

		#Compute comparation from first nearest and second nearest distance. If comparation less or equal than 0.7,
        #then take class from the first nearest distance. Else take frequently class.
		#Arousal
